...
8. sistema_matricial: a partir das equações MECÂNICAS do sub-bloco 7 (nunca das elétricas diretamente), monte a matriz de espaço de estados. Escolha o vetor de estado com posição/ângulo e velocidade de cada elemento de armazenamento de energia (ex. para 2 graus de liberdade: $x = [\\theta_1, \\omega_1, \\theta_2, \\omega_2]^T$, ordem 4). Monte $$\\dot{x} = Ax + Bu$$ e $$y = Cx + Du$$ — se a equação mecânica reduzida for uma EDO de ordem N de saída única, use a forma canônica controlável (última linha de A com os coeficientes da equação característica com sinal trocado, 1's na subdiagonal; B só na última linha; C seleciona o estado pedido). Explique de onde veio cada coeficiente — nunca apresente a matriz pronta sem essa ligação. A ORDEM da matriz é o número de variáveis de estado (não precisa ter relação com o tamanho de nenhuma matriz elétrica intermediária usada antes).
