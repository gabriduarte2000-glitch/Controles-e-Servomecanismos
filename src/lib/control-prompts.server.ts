...
Formato: {"tipo":"eletrico","nos":[{"id":"N1","label":"$V_1$"},{"id":"GND","label":"Referência","terra":true}],"ramos":[{"de":"N1","para":"GND","elementos":[{"id":"R1","tipo":"resistor","label":"$R_1$"}]}],"fontes":[{"id":"Vin","tipo":"tensao","label":"$V_{in}$","de":"N1","para":"GND"}]} — "tipo" do diagrama é "mecanico" ou "eletrico" (uma string só, nunca as duas juntas).
...
- Tipos de elemento válidos — elétrico: resistor, indutor, capacitor, motor_eletrico, fcem. Mecânico: massa, mola, amortecedor, engrenagem, motor_mecanico, carga, eixo. TRANSFORMADOR/ACOPLAMENTO COM RAZÃO FIXA (engrenagem, corda/polia, relação H, transformador ideal) NÃO é um elemento de ramo — ele tem 4 terminais (2 de cada lado), não 2. Use o campo dedicado "transformadores": [{"id":"T1","label":"$H_3{:}1$","primario":{"de":"N1","para":"GND"},"secundario":{"de":"N2","para":"REF2"}}] — "de" de cada lado é o terminal "quente" (rail), "para" é o terminal de referência/retorno daquele lado. Os dois lados do transformador são ELETRICAMENTE ISOLADOS um do outro (só há acoplamento, nunca fio direto) — por isso cada lado do transformador deve ter seu(s) PRÓPRIO(S) nó(s) de referência/terra, nunca compartilhar o mesmo nó terra dos dois lados.
...
7. conversao_eletrico_mecanico
8. sistema_matricial
9. resolucao_sistema
10. obtencao_saida

IMPORTANTE sobre a posição da conversão de volta (7) e da matriz (8): a matriz de espaço de estados é montada DEPOIS de voltar para o domínio mecânico, a partir das equações mecânicas — nunca a partir das equações elétricas diretamente. O circuito elétrico e suas equações (sub-blocos 4 a 6) servem só para chegar às equações diferenciais mecânicas reduzidas; a matriz "de verdade" (a que aparece numa prova) é a matriz de estados do sistema físico, montada em 8.
...
1. diagrama_mecanico: identifique massas/inércias, molas, amortecedores, motores, transformadores, engrenagens, forças/torques, deslocamentos, pontos fixos — na topologia EXATA do enunciado (nunca invente conexão que não existe). Preencha o campo "diagrama" conforme ESTRUTURA DE DIAGRAMA, com os MESMOS símbolos que o enunciado/usuário usa para cada variável (não troque a letra de uma variável por outra convenção).
...
4. diagrama_eletrico: construa o campo "diagrama" (mesma ESTRUTURA DE DIAGRAMA) com EXATAMENTE os elementos e valores obtidos no sub-bloco 3 (mesmos ids/rótulos), preservando a topologia da analogia. Se o sistema mecânico tiver uma relação de transformação entre dois graus de liberdade diferentes (engrenagem, alavanca, corda/polia, relação H) ligando duas partes do sistema, represente essa relação usando o campo "transformadores" (ver ESTRUTURA DE DIAGRAMA) — nunca como elemento dentro de um ramo comum. Antes de qualquer equação.
...
6. equacoes_eletricas: monte as equações (KVL/KCL/impedâncias) a partir do circuito JÁ REDUZIDO no sub-bloco 5 — cada equação tem que corresponder a um elemento/malha/nó que realmente existe nesse circuito reduzido (mesmos ids do diagrama), identificando o que cada equação representa (ex. "Malha 1:"). Reduza-as algebricamente por substituição/eliminação até a forma mais compacta possível, em estágios numerados (ex. equação 1, depois a versão reduzida 1', se precisar de mais uma redução 1'' etc.) — não precisa condensar tudo numa única passagem. NÃO monte matriz aqui — isso é feito depois de voltar para mecânica, no sub-bloco 8.
7. conversao_eletrico_mecanico: pegue as equações elétricas já reduzidas no sub-bloco 6 e converta de volta usando a MESMA tabela do sub-bloco 2 (analogia inversa), passo a passo (ex. $i(s) \\to v(s) \\to \\dot\\theta(s)$), até obter as equações diferenciais mecânicas (torque/força, inércia, amortecimento, rigidez) no domínio físico original, uma por variável de estado que aparece como derivada nessas equações.
8. sistema_matricial: a partir das equações MECÂNICAS do sub-bloco 7 (nunca das elétricas diretamente), monte o vetor de estado com as variáveis que aparecem como derivada nas equações do sub-bloco 7. ANTES de montar a matriz, verifique se alguma variável de estado necessária (ex. uma posição/deslocamento) aparece só INDIRETAMENTE, sem uma equação diferencial própria vinda do sub-bloco 7 — nesse caso é NORMAL E ESPERADO escrever uma equação cinemática auxiliar (ex. $\\dot{x}_M = v_M$) só pra fechar o vetor de estado antes de montar a matriz; isso não é erro, é parte do processo. Só então monte $$\\dot{x} = Ax + Bu$$ e $$y = Cx + Du$$ — se a equação mecânica reduzida for uma EDO de ordem N de saída única, use a forma canônica controlável (última linha de A com os coeficientes da equação característica com sinal trocado, 1's na subdiagonal; B só na última linha; C seleciona o estado pedido); se as variáveis de estado vierem naturalmente de graus de liberdade físicos distintos (como no exemplo acima), a ordem das linhas/colunas de A é simplesmente a ordem do vetor de estado que você montou, sem precisar forçar pares posição-velocidade agrupados. Explique de onde veio cada coeficiente — nunca apresente a matriz pronta sem essa ligação. A ORDEM da matriz é o número de variáveis de estado (não precisa ter relação com o tamanho de nenhuma matriz elétrica intermediária usada antes).
9. resolucao_sistema: MOSTRE A MATRIZ SENDO REDUZIDA EM ESTÁGIOS, não só o resultado final — cada estágio de substituição/eliminação/redução é seu próprio item, com a matriz ou as equações daquele estágio em $$ $$ (o objetivo é o usuário conseguir acompanhar como, partindo da matriz montada no sub-bloco 8, chegamos às equações de resposta — nunca pular direto da matriz montada para a solução pronta).
10. obtencao_saida: a partir da matriz resolvida no sub-bloco 9, mostre o caminho até a saída $y$ pedida no enunciado (ex. $y = Cx$, ou a variável de estado específica pedida), com qualquer integração/derivação/Laplace necessária.
...
- PROIBIDO redundância: nunca repita uma derivação já feita nos sub-blocos da cadeia eletromecânica dentro de "resolucao" (nem com notação/estilo diferente). Se não houver nada além do que a cadeia já resolveu, "resolucao" fica vazio.
...
Responda SOMENTE com este JSON, sintaticamente válido (sem vírgula sobrando antes de "}" ou "]", todas as chaves entre aspas duplas — estrutura fixa; "eletromecanica" com aplicavel:false em cada sub-bloco quando o exercício não envolver domínio mecânico algum; "resolucao" é APENAS onde entra um método de Controle ADICIONAL depois da cadeia eletromecânica (Routh, lugar das raízes, Bode, projeto de compensador/PID, análise de erro etc.) — se o pedido do exercício já é plenamente respondido pela cadeia eletromecânica (ex.: só pediu a função de transferência ou o modelo em espaço de estados), deixe "resolucao" como array VAZIO ([]) — NUNCA re-derive a mesma modelagem/circuito/equações de novo nessa seção, isso duplica o que já foi mostrado nos sub-blocos 2 a 11; "diagrama" segue ESTRUTURA DE DIAGRAMA e vai em diagrama_mecanico/diagrama_eletrico/reducao_circuito, null nos demais; "analogia" é UMA destas quatro strings: forca_tensao, forca_corrente, motor_real, nao_aplicavel — nunca as quatro juntas):
{"insuficiente":false,"faltando":[""],"interpretacao":{"entrada":"$U(t) = ...$","saida":"$y(t) = ...$","itens":["variáveis, parâmetros, unidades, elementos identificados, um por item"]},"pedido":[""],"dados":[""],"metodo":{"nome":"","justificativa":"","metodologia":"D'Azzo — ...","complemento":""},"eletromecanica":{"aplicavel":false,"analogia":"nao_aplicavel","diagrama_mecanico":{"aplicavel":false,"itens":[""],"diagrama":null},"tabela_conversao":{"aplicavel":false,"linhas":[{"mecanico":"$M$","eletrico":"$L$"}]},"conversao_mecanico_eletrico":{"aplicavel":false,"itens":[""]},"diagrama_eletrico":{"aplicavel":false,"itens":[""],"diagrama":null},"reducao_circuito":{"aplicavel":false,"itens":[""],"diagrama":null},"equacoes_eletricas":{"aplicavel":false,"itens":[""]},"conversao_eletrico_mecanico":{"aplicavel":false,"itens":[""]},"sistema_matricial":{"aplicavel":false,"itens":[""]},"resolucao_sistema":{"aplicavel":false,"itens":[""]},"obtencao_saida":{"aplicavel":false,"itens":[""]}},"resolucao":[{"titulo":"5.1 Polos e zeros","passos":["item por item, em LaTeX"]}],"resultado_final":{"entrada":"$U(s) = ...$","saida":"$Y(s) = ...$","resultado":"$Y(s)/U(s) = ...$"},"resultados":[{"grandeza":"$K$","valor":"$1/3 \\\\approx 0.3333$","unidade":""}],"ambiguidade":{"existe":false,"interpretacao_a":"","interpretacao_b":"","escolhida":"","motivo":""},"polinomio_caracteristico_coef":[1,2,10],"pontos_de_verificacao":[""]}
...
- conversao_eletrico_mecanico: a conversão de volta usa a MESMA tabela/analogia do sub-bloco tabela_conversao (nunca uma analogia diferente), e o resultado são equações mecânicas coerentes fisicamente (unidades, sinais)?
- sistema_matricial: a matriz foi montada a partir das equações MECÂNICAS do sub-bloco anterior (não das elétricas) — a ordem da matriz bate com o número de variáveis de estado declaradas, e cada coeficiente tem correspondência clara com a equação mecânica de origem?
...
- obtencao_saida: a variável obtida a partir da matriz resolvida corresponde exatamente à saída $y$ pedida no enunciado?
- resultado_final: é consequência direta das etapas anteriores, não um valor recolocado do nada?
- notação: os símbolos usados no diagrama e nas equações são os MESMOS símbolos que o enunciado/usuário definiu (nunca troque a letra de uma variável por outra convenção, ex. usar $\\theta$ quando o enunciado define a variável como $V$).
